...
Comprehensive data quality testing framework for ETL pipeline validation:

**Test Data Generation:**
- `generate_sales_orders_csv.py` generates 791,532 orders with configurable data quality issues
- 19 hyperparameters control injection of various data quality problems
- Validation manifest (JSON) documents expected issue counts

**Test Categories:**

1. **Missing/Null Values (6 parameters):**
   - Customer IDs, order dates, tax, invoice numbers, payment methods, subtotals
   - Tests ETL null handling and schema inference
   - Validates Glue Crawler behavior with sparse data

2. **Invalid Data Formats (4 parameters):**
   - Malformed dates (e.g., "2024-13-45", "invalid-date")
   - Whitespace in IDs (leading/trailing spaces)
   - Formatted numbers with commas ("1,234.56")
...
3. **Edge Cases (5 parameters):**
   ...
   - Negative amounts (business logic violations)
   - Out-of-range dates (future/past)
   - Zero quantities in line items
   - Tests constraint enforcement and business rules

**Testing Workflow:**
1. Generate test data: `python generate_sales_orders_csv.py`
2. Upload via DataSync to S3 raw zone
3. Run Glue Crawler for schema inference
4. Execute ETL jobs with data quality rules
5. Compare actual vs. expected issues from manifest
6. Validate data cleansing and transformation logic

**Key Validations:**
- Schema inference with high null rates
- Type detection (STRING vs. DOUBLE/TIMESTAMP)
- Duplicate detection and handling
- Invalid data rejection or cleansing
- Out-of-range data filtering

**Documentation:**
- `DATA_QUALITY_TESTING.md` - Comprehensive 326-line testing guide
- `DATA_QUALITY_QUICK_REFERENCE.md` - 136-line quick reference

See data quality testing documentation for complete parameter descriptions, test scenarios, and AWS Glue-specific considerations.
...
### Data Quality Testing
- `DATA_QUALITY_TESTING.md` - Comprehensive ETL testing guide (326 lines)
- `DATA_QUALITY_QUICK_REFERENCE.md` - Quick reference for DQ parameters (136 lines)

### Related Tools
- `generate_sales_orders_csv.py` - Sales order generator with data quality testing features
  - Generates 791,532 orders with configurable data quality issues
  - Tests ETL pipeline robustness (missing values, invalid data, edge cases)
  - Outputs validation manifest for expected vs. actual issue tracking
